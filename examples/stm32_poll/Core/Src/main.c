...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
	uint16_t reg = 1; //number holding register

#ifdef NMBS_DEBUG
    printf("Begin read holding register %d from %d on spped%d\n", reg,RTU_SERVER_ADDRESS,Speed);
    printf("Address^ %d\n", RTU_SERVER_ADDRESS);
    printf("Speed%d\n", Speed);
#endif
